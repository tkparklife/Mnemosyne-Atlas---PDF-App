    ...
    const renderPage = async () => {
      ...
      try {
        ...
        if (active) {
          ...
          setErrorMessage(
            err.message ||
              "Error occurred while drawing PDF visual canvas page",
          );
        }
      }
    };
    ...
    >
      ...
      {renderState === "loading" && (
        <div className="absolute inset-0 flex flex-col items-center justify-center bg-white/70 backdrop-blur-sm z-15 gap-2 select-none">
          ...
          <span className="text-[10px] font-mono text-indigo-700 uppercase tracking-widest">
      ...
      {renderState === "error" && (
        <div className="absolute inset-0 flex flex-col items-center justify-center bg-slate-50 border border-rose-200 p-4 text-center z-15 select-none">
          ...
          <div className="text-rose-600 text-[10px] font-bold font-mono uppercase tracking-wider mb-1">
            Canvas Draw Error
          </div>
          <div className="text-slate-600 text-[10.5px] font-sans max-w-xs">
            {errorMessage}
          </div>
        </div>
      )}
      ...
      <div
        ...
        style={{
          width:
            renderState === "success" && viewport ? viewport.width : "auto",
          height:
            renderState === "success" && viewport ? viewport.height : "auto",
          maxWidth: "100%",
      ...
      >
        ...
        {renderState === "success" && viewport && textItems.length > 0 && (
          <div className="z-10 pointer-events-auto select-text overflow-hidden pdf-text-layer-overlay">
            <style>{`
...
export default function CenterPanel({
  ...
  const [selectedColor, setSelectedColor] = useState(
    "bg-yellow-105 border-yellow-300 ring-yellow-400",
  );
  ...
  if (!document) {
    return (
      <div
        className="flex-1 flex flex-col items-center justify-center bg-[#E5E7EB] p-8 border-r border-slate-200"
        id="center-panel-root"
      >
        <div className="text-center max-w-sm font-sans">
          ...
          <p className="text-xs text-slate-500 leading-relaxed mt-1">
            Pick an item from the library on the left or upload a raw file to
            run the Gemini indexing pipeline.
          </p>
        </div>
      </div>
    );
  ...
  const pageHighlights = highlights.filter(
    (h) => h.documentId === document.id && h.pageNumber === activePageNumber,
  );

  return (
    <div
      className="flex-1 flex flex-col bg-[#E5E7EB] border-r border-slate-200 overflow-hidden relative h-full"
      id="center-panel-root"
    >
      ...
      <div className="h-12 bg-[#FAFAFA] border-b border-slate-200 flex items-center justify-between px-4 shrink-0 shadow-sm z-10">
        <div className="flex items-center gap-2 min-w-0 font-sans">
          ...
          <h2
            className="text-xs font-bold text-slate-900 truncate animate-none"
      ...
      >
        <div className="w-full max-w-[1200px] flex flex-col items-center relative gap-8 box-border">
          {(() => {
            const pg =
              document.pages.find((p) => p.pageNumber === activePageNumber) ||
              document.pages[0];
            if (!pg) return null;
            const pageHighlights = highlights.filter(
              (h) =>
                h.documentId === document.id && h.pageNumber === pg.pageNumber,
            );
                ...
                <div className="border-b border-slate-100 pb-3 mb-4 flex items-center justify-between shrink-0 select-none">
                  <span className="text-[10px] uppercase tracking-widest font-mono text-indigo-500 font-bold">
                    {document.fileType === "image"
                      ? "Image Scan Layer"
                      : `Visual Document Page ${pg.pageNumber} of ${document.totalPages}`}
                  </span>
                  ...
                  {document.fileData && document.fileType === "pdf" ? (
                    ...
                    <div className="relative w-full h-full min-h-[600px] p-8 md:p-12 bg-[#FCFAF2] text-[#2C241E] flex flex-col justify-between select-text font-serif shadow-inner border border-[#E8DFC8]/40 rounded-lg box-border">
                      <div className="absolute top-4 left-6 right-6 flex items-center justify-between text-[11px] font-sans text-[#7D6B58] uppercase tracking-wider select-none border-b border-[#E8DFC8]/30 pb-2">
                        <span className="font-semibold text-indigo-900/50">
                          Manuscript Archive Record
                        </span>
                        <span className="font-mono text-[#7D6B58]/60">
                          PAGE {pg.pageNumber}
                        </span>
                      </div>
                      ...
                      <div className="absolute bottom-4 left-6 right-6 border-t border-[#E8DFC8]/30 pt-2 text-[10px] font-serif italic text-[#7D6B58] text-center select-none">
                        Chrono-Core Historical Library • Item Index #
                        {document.id}
                      </div>
                    </div>
                  )}
                  ...
                  <div className="mt-4 pt-4 border-t border-slate-100 font-sans select-none">
                    ...
                    {pg.summary && (
                      <p className="text-[10px] text-slate-600 italic mt-2.5 border-t border-slate-100 pt-2 leading-snug">
                        <span className="font-bold uppercase text-[8px] text-slate-500 mr-1.5 bg-slate-50 border border-slate-200 px-1 py-0.5 rounded font-mono">
                          Page Thesis:
                        </span>
                        "{pg.summary}"
                      </p>
                  ...
                  <div className="mt-4 pt-4 border-t border-slate-100 select-none font-sans">
                    ...
                    <div className="space-y-2">
                      {pageHighlights.map((hl) => (
                        <div
                          key={hl.id}
                          className="text-[11px] text-slate-700 bg-slate-50 border border-slate-200 p-2.5 rounded shadow-sm flex justify-between gap-4"
                        >
      ...
      {showTooltip && (
        ...
        >
          {!showCommentInput ? (
            ...
            <form
              onSubmit={executeAddHighlight}
              className="p-1 flex flex-col gap-2 w-52 text-xs font-sans"
            >
